...
// NBA V2: these were `const` in V1 (single device, single business). They
// are now `let` so applyUserScopeToStorageKeys() (defined near the auth
// code, below) can re-point every one of them to a per-user-scoped key the
// moment someone signs in — WITHOUT changing any of the ~50 call sites
// throughout this file that already reference these names. See the "NBA
// V2 — Multi-user data isolation" section near the bottom of this file for
// the full explanation of the bug this fixes and why this approach is safe.
let STORAGE_KEY = "nexaBusinessProfile";
let SALES_KEY = "nexaSales";
let INVENTORY_KEY = "nexaInventory";
let SUPPLIERS_KEY = "nexaSuppliers";
let PURCHASES_KEY = "nexaPurchases";
let SUPPLIER_PAYMENTS_KEY = "nexaSupplierPayments"; // reserved for a future "record payment" feature
let EXPENSES_KEY = "nexaExpenses";
let EXPENSE_COUNTER_KEY = "nexaExpenseCounter"; // only ever increases, so expense numbers stay unique even after deletes
let DRAWINGS_KEY = "nexaDrawings";
let DRAWING_COUNTER_KEY = "nexaDrawingCounter"; // only ever increases, so drawing numbers stay unique even after deletes
let CUSTOMERS_KEY = "nexaCustomers";
let CUSTOMER_PAYMENTS_KEY = "nexaCustomerPayments";
let RECEIPT_NUMBERS_KEY = "nexaReceiptNumbers"; // saleId -> "NBA-000001", assigned once on first view, never reused
let RECEIPT_COUNTER_KEY = "nexaReceiptCounter";
let SETTINGS_KEY = "nexaSettings"; // Module 20 — configuration/display preferences only, never financial data
let INVENTORY_ADJUSTMENTS_KEY = "nexaInventoryAdjustments"; // Module 21 — manual stock adjustments, separate from Purchases/Sales
let ADJUSTMENT_COUNTER_KEY = "nexaAdjustmentCounter"; // only ever increases, so adjustment numbers stay unique even after reversal
let MONEY_TRANSFERS_KEY = "nexaMoneyTransfers"; // Module 22 — internal Cash <-> Bank transfers, separate from Sales/Purchases/Expenses
let TRANSFER_COUNTER_KEY = "nexaTransferCounter"; // only ever increases, so transfer numbers stay unique even after reversal
...
let LAST_BACKUP_KEY = "nexaLastBackupAt"; // metadata only — not business data, excluded from backups
...
// NBA V2 — a frozen snapshot of the ORIGINAL, unscoped V1 key names, taken
// right now at script load time (before any user signs in and before
// applyUserScopeToStorageKeys() ever runs). This is how real pre-existing
// V1 business data on a device gets found and offered for claiming later —
// see the "NBA V2 — Multi-user data isolation" section near the bottom of
// this file. NBA_BACKUP_KEYS itself gets its values REPLACED at login, so
// this separate copy is the only remaining record of where V1 data lives.
const LEGACY_UNSCOPED_KEYS = Object.assign({}, NBA_BACKUP_KEYS);
...
if (typeof firebase === "undefined" || window.__nbaFirebaseInitError) {
  ...
  firebase.auth().onAuthStateChanged(async (user) => {
    if (user) {
      // NBA V2 — MUST run before anything else touches storage. Re-points
      // every *_KEY variable (and NBA_BACKUP_KEYS) to this specific user's
      // own namespace. See "NBA V2 — Multi-user data isolation" below.
      applyUserScopeToStorageKeys(user.uid);
      ...
      // If this user has no business of their own yet, AND this device has
      // real (unscoped) V1 data nobody has claimed, offer it rather than
      // treating this user as having no data at all.
      if (!loadBusiness() && hasUnclaimedLegacyData()) {
        legacyClaimText.textContent =
          "We found \"" + getLegacyBusinessName() + "\" saved on this device from before multiple accounts were supported. Is this your business?";
        legacyClaimBanner.hidden = false;
      }
...
function setSyncStatus(status, errorDetail) {
  ...
  // NBA V2 — never hide the real error: put it in the `title` attribute so
  // it's reachable (tap-and-hold on mobile, hover on desktop) without
  // needing a developer console, while the on-screen label stays plain.
  syncIndicatorEl.title = status === "error" && errorDetail
    ? "Firestore error: " + (errorDetail.code || errorDetail.message || String(errorDetail))
    : "";
}

function logCloudError(context, err) {
  // Always logs the real Firebase/Firestore error — never suppressed.
  console.error("[NBA Cloud Sync] " + context + " — code: " + (err && err.code) + " — " + (err && err.message));
}
...
function pushBusinessSnapshotToCloud() {
  ...
  getBusinessDocRef().set({
    ...
    .catch((err) => {
      logCloudError("push", err);
      setSyncStatus(navigator.onLine ? "error" : "offline", err);
    });
}
...
async function initCloudSync() {
  ...
  try {
    ...
    logCloudError("initial connection", err);
    setSyncStatus(navigator.onLine ? "error" : "offline", err);
  ...
  if (cloudExists && cloudMigrated) {
    ...
    try {
      ...
      logCloudError("pull (established account)", err);
      setSyncStatus(navigator.onLine ? "error" : "offline", err);
    ...
    try {
      ...
      logCloudError("pull (new device)", err);
      setSyncStatus(navigator.onLine ? "error" : "offline", err);
    }
  } else {
...
conflictUseCloudBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    logCloudError("conflict resolution (use cloud)", err);
...
// ---- 13. NBA V2 — Multi-user data isolation --------------------------------
//
// THE BUG: every save*()/load*() function reads/writes plain global
// localStorage keys ("nexaSales", "nexaBusinessProfile", ...) that have
// no idea which Firebase user is signed in. On a shared device, a second
// person logging in would see the first person's business — because
// nothing ever distinguished them. Firestore was NOT the leak: businesses/
// {uid} was already correctly isolated per-user by construction. The hole
// was entirely in localStorage.
//
// THE FIX: the ~19 key constants at the top of this file (STORAGE_KEY,
// SALES_KEY, ...) were changed from `const` to `let`. applyUserScopeToStorageKeys()
// below reassigns every one of them — and NBA_BACKUP_KEYS' values — to a
// "nba:{uid}:..." namespaced string the moment a user signs in. Every one
// of the ~50 existing localStorage.getItem/setItem/removeItem call sites
// throughout the file is UNCHANGED — they just now resolve through a
// variable that points somewhere per-user, instead of a fixed global.
// This was chosen over shadowing the global `localStorage` object itself
// (which would also be visible to the Firebase SDK's own internal code —
// too risky to ship without being able to test it live).
//
// SCOPE NOTE: this keeps NBA's existing single-business-per-user model
// (Firebase UID doubles as the business ID, as originally implemented) —
// it does not add multi-business-per-user support. That's a materially
// larger feature (business switcher UI, membership/roles, a different
// Firestore/security-rules shape) and bolting it on inside an urgent data-
// isolation fix would make the fix itself harder to verify. Flagging this
// as a deliberate scope decision, not an oversight.

function applyUserScopeToStorageKeys(uid) {
  const prefix = "nba:" + uid + ":";

  STORAGE_KEY = prefix + "nexaBusinessProfile";
  SALES_KEY = prefix + "nexaSales";
  INVENTORY_KEY = prefix + "nexaInventory";
  SUPPLIERS_KEY = prefix + "nexaSuppliers";
  PURCHASES_KEY = prefix + "nexaPurchases";
  SUPPLIER_PAYMENTS_KEY = prefix + "nexaSupplierPayments";
  EXPENSES_KEY = prefix + "nexaExpenses";
  EXPENSE_COUNTER_KEY = prefix + "nexaExpenseCounter";
  DRAWINGS_KEY = prefix + "nexaDrawings";
  DRAWING_COUNTER_KEY = prefix + "nexaDrawingCounter";
  CUSTOMERS_KEY = prefix + "nexaCustomers";
  CUSTOMER_PAYMENTS_KEY = prefix + "nexaCustomerPayments";
  RECEIPT_NUMBERS_KEY = prefix + "nexaReceiptNumbers";
  RECEIPT_COUNTER_KEY = prefix + "nexaReceiptCounter";
  SETTINGS_KEY = prefix + "nexaSettings";
  INVENTORY_ADJUSTMENTS_KEY = prefix + "nexaInventoryAdjustments";
  ADJUSTMENT_COUNTER_KEY = prefix + "nexaAdjustmentCounter";
  MONEY_TRANSFERS_KEY = prefix + "nexaMoneyTransfers";
  TRANSFER_COUNTER_KEY = prefix + "nexaTransferCounter";
  LAST_BACKUP_KEY = prefix + "nexaLastBackupAt";

  // NBA_BACKUP_KEYS was built ONCE at script-load time from the original
  // (unscoped) values of the variables above — object property values are
  // copied at construction, not live-linked — so it needs its own values
  // refreshed here too. Without this, Backup/Restore and all of Cloud Sync
  // (section 12, which reads/writes exclusively through NBA_BACKUP_KEYS)
  // would silently keep using the unscoped V1 keys forever.
  NBA_BACKUP_KEYS.business = STORAGE_KEY;
  NBA_BACKUP_KEYS.sales = SALES_KEY;
  NBA_BACKUP_KEYS.inventory = INVENTORY_KEY;
  NBA_BACKUP_KEYS.suppliers = SUPPLIERS_KEY;
  NBA_BACKUP_KEYS.purchases = PURCHASES_KEY;
  NBA_BACKUP_KEYS.supplierPayments = SUPPLIER_PAYMENTS_KEY;
  NBA_BACKUP_KEYS.expenses = EXPENSES_KEY;
  NBA_BACKUP_KEYS.expenseCounter = EXPENSE_COUNTER_KEY;
  NBA_BACKUP_KEYS.drawings = DRAWINGS_KEY;
  NBA_BACKUP_KEYS.drawingCounter = DRAWING_COUNTER_KEY;
  NBA_BACKUP_KEYS.customers = CUSTOMERS_KEY;
  NBA_BACKUP_KEYS.customerPayments = CUSTOMER_PAYMENTS_KEY;
  NBA_BACKUP_KEYS.receiptNumbers = RECEIPT_NUMBERS_KEY;
  NBA_BACKUP_KEYS.receiptCounter = RECEIPT_COUNTER_KEY;
  NBA_BACKUP_KEYS.settings = SETTINGS_KEY;
  NBA_BACKUP_KEYS.inventoryAdjustments = INVENTORY_ADJUSTMENTS_KEY;
  NBA_BACKUP_KEYS.adjustmentCounter = ADJUSTMENT_COUNTER_KEY;
  NBA_BACKUP_KEYS.moneyTransfers = MONEY_TRANSFERS_KEY;
  NBA_BACKUP_KEYS.transferCounter = TRANSFER_COUNTER_KEY;

  // The settings cache is an in-memory JS variable, not localStorage — if a
  // second user signs in during the same page session (logout then login
  // as someone else, no full reload), a stale cached object from the
  // PREVIOUS user would otherwise leak through loadSettings() even after
  // the keys above are correctly re-scoped.
  cachedSettings = null;
}

// ---- 13a. Legacy V1 data claim ----
// Real pre-existing business data (from before accounts existed) still
// sits under the original unscoped keys on any device that used NBA
// before this update. It must be offered to its real owner explicitly —
// never auto-assigned to whoever happens to log in next.

const LEGACY_CLAIM_FLAG_KEY = "nbaLegacyDataClaimedBy"; // deliberately NEVER scoped — device-wide, not per-user

const legacyClaimBanner = document.getElementById("legacyClaimBanner");
const legacyClaimText = document.getElementById("legacyClaimText");
const legacyClaimYesBtn = document.getElementById("legacyClaimYesBtn");
const legacyClaimNoBtn = document.getElementById("legacyClaimNoBtn");
const legacyClaimStatusText = document.getElementById("legacyClaimStatusText");

function hasUnclaimedLegacyData() {
  if (window.localStorage.getItem(LEGACY_CLAIM_FLAG_KEY)) return false; // already resolved once on this device
  return !!window.localStorage.getItem(LEGACY_UNSCOPED_KEYS.business);
}

function getLegacyBusinessName() {
  try {
    const raw = window.localStorage.getItem(LEGACY_UNSCOPED_KEYS.business);
    const parsed = raw ? JSON.parse(raw) : null;
    return (parsed && parsed.businessName) || "your business";
  } catch (e) {
    return "your business";
  }
}

function claimLegacyDataForCurrentUser() {
  Object.entries(LEGACY_UNSCOPED_KEYS).forEach(([field, legacyKey]) => {
    const value = window.localStorage.getItem(legacyKey);
    if (value !== null) {
      const scopedKey = NBA_BACKUP_KEYS[field]; // already re-scoped to the current user by this point
      window.localStorage.setItem(scopedKey, value);
    }
  });
  window.localStorage.setItem(LEGACY_CLAIM_FLAG_KEY, currentNBAUser.uid);
}

legacyClaimYesBtn.addEventListener("click", () => {
  legacyClaimYesBtn.disabled = true;
  legacyClaimNoBtn.disabled = true;
  legacyClaimStatusText.hidden = false;
  legacyClaimStatusText.textContent = "Loading your business…";

  claimLegacyDataForCurrentUser();
  // Reload for a clean re-entry through the whole auth/cloud-sync flow,
  // now that this user's scoped storage actually has data — the same
  // pattern used by the cloud-conflict banner above.
  window.location.reload();
});

legacyClaimNoBtn.addEventListener("click", () => {
  legacyClaimBanner.hidden = true;
  // Not permanently dismissed — real unclaimed business data sitting on a
  // device is worth resurfacing next login rather than silently forgetting,
  // same reasoning as the cloud migration banner in section 12.
});
